...
<head runat="server">
    ...
    <style>
        ...
        /* SIDEBAR */
        .sidebar {
            min-width: 260px; max-width: 260px; background: #212529; color: #fff;
            transition: all 0.3s; box-shadow: 4px 0 15px rgba(0,0,0,0.1); display: none;
        }
        .sidebar.open { display: block; }
        .sidebar-header { padding: 25px 20px; background: #1b1e21; border-bottom: 1px solid #32383e; }
        .sidebar ul.components { padding: 15px 0; border-bottom: none; }
        .sidebar ul li a {
            padding: 16px 25px; font-size: 1rem; display: flex; align-items: center;
            color: #adb5bd; text-decoration: none; transition: all 0.3s; border-left: 4px solid transparent;
        }
        .sidebar ul li a:hover { color: #fff; background: #2c3034; border-left: 4px solid #6c757d; }
        .sidebar ul li.active > a {
            color: #fff; background: linear-gradient(90deg, #0d6efd 0%, #0a58ca 100%);
            border-left: 4px solid #6610f2;
        }
        ...
        .stat-card {
            ...
            cursor: pointer; border-left: 5px solid #ccc; padding: 15px;
        }
        ...
        .custom-table thead { background-color: #f8f9fa; border-bottom: 2px solid #dee2e6; }
        .custom-table th, .custom-table td { padding: 15px 20px; vertical-align: middle; }

        /* Badges */
        .freq-badge { padding: 5px 10px; border-radius: 20px; font-size: 0.85em; font-weight: 600; }
...
<body>
    <form id="form1" runat="server">
        <div class="wrapper">
            <nav class="sidebar" id="sidebar">
                <div class="sidebar-header"><h4>Menu</h4></div>
                <ul class="list-unstyled components">
                    <li id="liTasks" runat="server" class="active">
                        <asp:LinkButton ID="BtnNavTasks" runat="server" OnClick="BtnNavTasks_Click">
                            <i class="fa fa-list-check sidebar-icon"></i> Tasks
            ...
            <div class="content">
                <asp:MultiView ID="MainMultiView" runat="server" ActiveViewIndex="0">
                    ...
                    <asp:View ID="ViewTasks" runat="server">
                        <div class="page-header-bar">
                            <div style="display:flex; gap:15px; align-items:center;">
                                <button type="button" id="btnSidebarToggle" class="btn btn-outline-secondary"><i class="fa fa-bars"></i></button>
                                <h2 class="m-0">Infinite CareSource Automation</h2>
                            </div>
                            </div>

                        <div class="row mb-4">
                            <div class="col-md-3"><div class="stat-card" onclick="filterTable('all')"><h6>Total</h6><h3><asp:Label ID="lblTotal" runat="server">0</asp:Label></h3></div></div>
                            <div class="col-md-3"><div class="stat-card card-daily" onclick="filterTable('Daily')"><h6 class="text-success">Daily</h6><h3><asp:Label ID="lblDaily" runat="server">0</asp:Label></h3></div></div>
                            <div class="col-md-3"><div class="stat-card card-weekly" onclick="filterTable('Weekly')"><h6 class="text-warning">Weekly</h6><h3><asp:Label ID="lblWeekly" runat="server">0</asp:Label></h3></div></div>
                            <div class="col-md-3"><div class="stat-card card-monitor" onclick="filterTable('Monitoring')"><h6 class="text-info">Monitoring</h6><h3><asp:Label ID="lblMonitor" runat="server">0</asp:Label></h3></div></div>
                        </div>
                        ...
                        <div class="custom-table">
                            <asp:GridView ID="GridTasks" runat="server" AutoGenerateColumns="false" CssClass="table table-hover mb-0" GridLines="None">
                                <Columns>
                                    <asp:BoundField HeaderText="Task Name" DataField="Name" ItemStyle-Font-Bold="true" />
                                    <asp:TemplateField HeaderText="Frequency">
                                        <ItemTemplate><span class="freq-badge"><%# Eval("Frequency") %></span></ItemTemplate>
                                    </asp:TemplateField>
                                    <asp:TemplateField HeaderText="Status">
                                        <ItemTemplate>
                                            <asp:DropDownList ID="ddlStatus" runat="server" CssClass="form-select form-select-sm status-dropdown" onchange="updateStatusColor(this)">
                                    ...
                                    <asp:TemplateField HeaderText="Action">
                                        <ItemTemplate><%# GetFolderLink(Eval("FolderPath")) %></ItemTemplate>
                                    </asp:TemplateField>
                                </Columns>
                            </asp:GridView>
                        </div>
                    ...
                    <asp:View ID="ViewDocs" runat="server">
                        <div class="page-header-bar">
                             <div style="display:flex; gap:15px; align-items:center;">
                                <button type="button" class="btn btn-outline-secondary" onclick="document.getElementById('sidebar').classList.toggle('open')"><i class="fa fa-bars"></i></button>
                        ...
                        <div class="custom-table">
                            <asp:GridView ID="GridDocs" runat="server" AutoGenerateColumns="false" CssClass="table table-hover mb-0" GridLines="None">
                                <Columns>
                                    ...
                                    <asp:BoundField HeaderText="Path" DataField="DocPath" ItemStyle-CssClass="text-muted small" />
                                    <asp:TemplateField HeaderText="Access">
                                        <ItemTemplate><%# GetFolderLink(Eval("DocPath"), "View Docs") %></ItemTemplate>
                                    </asp:TemplateField>
                                </Columns>
                            </asp:GridView>
                        </div>
    ...
    <script>
        // 1. Sidebar Toggle
        var btn = document.getElementById("btnSidebarToggle");
        if (btn) { btn.addEventListener("click", function () { document.getElementById("sidebar").classList.toggle("open"); }); }
        ...
        function filterTable(category) {
            ...
            for (var i = 1; i < rows.length; i++) {
                ...
                if (cell) {
                    ...
                    rows[i].style.display = (category === 'all' || txt.includes(category)) ? "" : "none";
                }
            }
        }
        ...
        function updateStatusColor(ddl) {
            var val = ddl.value;
            ddl.style.borderColor = val === "Completed" ? "#198754" : (val === "In Progress" ? "#ffc107" : "#dc3545");
            ddl.style.color = val === "Completed" ? "#fff" : (val === "In Progress" ? "#856404" : "#dc3545");
            ddl.style.backgroundColor = val === "Completed" ? "#198754" : "#fff";
        }

        // Init on load
        document.addEventListener("DOMContentLoaded", function () {
            // Badges
            document.querySelectorAll('.freq-badge').forEach(function (b) {
                var t = b.innerText;
                if (t.includes("Daily")) b.className += " badge-soft-success";
                else if (t.includes("Weekly")) b.className += " badge-soft-warning";
                else b.className += " badge-soft-info";
            });
            // Dropdowns
            document.querySelectorAll('.status-dropdown').forEach(function (d) { updateStatusColor(d); });
        });
    </script>
</body>
</html>--%>
...
<head runat="server">
    ...
    <style>
        ...
        /* --- SIDEBAR STYLES (UPDATED TO VIOLET) --- */
        .sidebar {
            min-width: 260px; max-width: 260px;
            /* Change background to Light Violet */
            background: #212529;
            color: #fff;
            transition: all 0.3s; box-shadow: 4px 0 15px rgba(0,0,0,0.1); display: none;
        }
        ...
        .sidebar-header {
            padding: 25px 20px;
            /* Slightly darker violet for the header for depth */
            background: #735A99; 
            /* Lighter border to match */
            border-bottom: 1px solid #9B7FC6; 
        }
        .sidebar ul.components { padding: 15px 0; border-bottom: none; }
        
        .sidebar ul li a {
            padding: 16px 25px; font-size: 1rem; display: flex; align-items: center;
            /* Lighter text color for inactive links against violet */
            color: #eaddfd; 
        ...
        .sidebar ul li a:hover {
            color: #fff;
            background: #735A99; /* Darker violet on hover */
            border-left: 4px solid #d8b4fe; /* Light accent border */
        }
        
        /* Active state - kept the blue gradient as it contrasts well, 
           but you could change this to a violet gradient if preferred */
        .sidebar ul li.active > a {
            color: #fff;
            background: linear-gradient(90deg, #0d6efd 0%, #0a58ca 100%);
            border-left: 4px solid #6610f2;
                ...
                <asp:MultiView ID="MainMultiView" runat="server" ActiveViewIndex="0">
                    ...
                    <asp:View ID="ViewTasks" runat="server">
                        <div class="page-header-bar">
                            ...
                            <div style="display:flex; gap:15px; align-items:center;">
                                <img src="Images/logo.png" alt="Logo 1" style="height: 50px; object-fit: contain;" />
                                <img src="Images/Logo1.png" alt="Logo 2" style="height: 50px; object-fit: contain;" />
                            </div>
                        </div>
